...
def NOTIFIED_ADD_CONTACT(op):
    try:
        sendMessage(op.param1, client.getContact(op.param1).displayName + "Thanks for add me, My Creator Http://line.me/ti/p/~lianekof")
        ...
        if msg.toType == 2:
            if msg.contentType == 0:
                        ...
                        if targets == []:
                            ...
                            for target in targets:
                                try:
                                    ...
                                    print (msg.to,[g.mid])
                if "Cn " in msg.text:                    
                    X = client.getProfile(msg.to)
                    profile = client.getProfile()
                    X.name = msg.text.replace("Cn ","")                    
                    client.updateProfile(profile)                    
                if "My speed" in msg.text:
                    start = time.time()
                    ...
                    if targets == []:
                        ...
                        for target in targets:
                            try:
                                ...
                                client.sendText(msg.to,"Succes Ian")
                                client.sendText(msg.to,"Fuck You")                    
                if msg.text == "cancel":
                    group = client.getGroup(msg.to)
                ...
                if msg.text == "Set":
                    ...
                    wait['setTime'][msg.to] = datetime.datetime.today().strftime('%Y-%m-%d Jam %H:%M:%S')
                    wait['ROM'][msg.to] = {}
                    print wait
                if msg.text == "Cek":
                    if msg.to in wait['readPoint']:
                        ...
                        sendMessage(msg.to, "Tercyduck %s\n\n\nSider Read Tapi Ga Ikut Chatt\n%s\n\nSetpoint Pada:\n[%s]"  % (wait['readMember'][msg.to],chiya,setTime[msg.to]))
                    else:
                        sendMessage(msg.to, "Sepoint Dulu Coeg")
                else:
                    pass
        else:
            pass
